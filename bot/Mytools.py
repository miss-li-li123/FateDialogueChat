...
import re
import jieba
YUANFENJU_API_KEY = "K0I5WCmce7jlMZzTw7vi1xsn0"
...
def get_info_from_local_db(query:str):
    """只有回答与2024年运势或龙年运势相关的问题的时候，会使用这个工具"""
    client = Qdrant(
        QdrantClient("/Users/josh/Desktop/aiProject/FateDialogueChat/bot/local_qdrand"),
        "local_documents",
...
def bazi_cesuan(query:str):
    ...
    prompt = ChatPromptTemplate.from_template(
        ...
        JSON字段如下： -"api_key":{YUANFENJU_API_KEY}, - "name":"姓
        名", - "sex":"性别，0表示男，1表示女，根据姓名判断", - "type":"日历类
        型，0农历，1公里，默认1"，- "year":"出生年份 例：1998", - "month":"出
        生月份 例 8", - "day":"出生日期，例：8", - "hours":"出生小时 例 14", 
        - "minute":"0"，如果没有找到相关参数，则需要提醒用户告诉你这些内容，只返回
        数据结构，不要有其他的评论，用户输入:{query}""")

    parser = JsonOutputParser()
    prompt = prompt.partial(format_instructions=parser.get_format_instructions())
    chain = prompt | app_config.deepseek.create_chat_model() | parser
    ...
    if result.status_code == 200:
        ...
        try:
            json = result.json()
            bazi_list = json["data"]["bazi_info"]["bazi"]
            returnstring = f"八字为: {' '.join(bazi_list)}"
            return returnstring
        except Exception as e:
            return "查询失败,可能是你忘记询问用户姓名或者出生年月日时了。"
...
def jiemeng(query:str):
    ...
    LLM = app_config.deepseek.create_chat_model()
    prompt = PromptTemplate.from_template("请从以下梦境描述中提取1个核心中文关键词，用英文逗号分隔，不要空格。中文描述: {topic}")
    prompt_value = prompt.invoke({"topic":query})
    keyword = LLM.invoke(prompt_value)
    data={"api_key":api_key,"title_zhougong":keyword.content}
    print("请求数据:", data)
    result = requests.post(url,data=data)
    if result.status_code == 200:
        print("====返回数据=====")
